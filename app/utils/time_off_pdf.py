...
def build_time_off_pdf(request, settings=None) -> bytes:
    ...
    rows = [
        ...
        [_para("Submitted", label_style), _para(
    ...
    if request.reviewed_at and reviewer:
        ...
        elements.append(
            _para(f"Approved by {reviewer_name} on {reviewed_at}", value_style)
        )
    ...
    sig_rows = [
        ...
        [
            ...
            _para(
                f"Date: {request.reviewed_at.strftime('%Y-%m-%d')}" if request.reviewed_at else "Date: ___________________________",
                value_style,
            ),
        ],
        [
    ...
    elements.append(
        _para(f"Generated by TimeTracker on {generated}", ParagraphStyle(
            "Footer", fontName="Helvetica", fontSize=8, textColor=MUTED_TEXT, alignment=TA_LEFT
        ))
    )
