...
def test_user(db_session):
    ...
    Idempotent: reuse the existing 'testuser' if the global conftest user
    fixture already created it in the same test DB.
    """
    existing = User.query.filter_by(username="testuser").first()
    if existing:
        existing.role = "admin"
        existing.is_active = True
        existing.set_password("testpass")
        db_session.commit()
...
class TestCalDAVConnector:
    ...
    def test_sync_data_imports_events(self, mock_client_class, db_session, caldav_integration, test_project):
        ...
        calendar_event = CalendarEvent.query.filter_by(user_id=caldav_integration.user_id).first()
